...
Original file is located at
    https://colab.research.google.com/drive/1Dno2BoSSJSlc1Bof5ln_KkZZzcnFueah
...
class CustomEEGDataset(Dataset):
    def __init__(self, adhd_data, control_data, predicting, transform=None, target_transform=None):
      if predicting:
        x = [adhd_data]
        y = [control_data]
        self.eeg_labels = y
...
dataset =  CustomEEGDataset(adhd_splited_arrays, control_splited_arrays, False)
...
## Randomly select EEG from test dataset
"""

rnd_sample_idx = np.random.randint(len(test_loader.dataset))
label = test_loader.dataset[rnd_sample_idx][1]
print('El EEG muestreado representa un paciente: ' + ('CON' if label else 'SIN') + ' TDAH')
data = test_loader.dataset[rnd_sample_idx][0]

"""## Make a prediction for randomly selected EEG"""

pred_dataset =  CustomEEGDataset(data, label, True)
pred_loader = DataLoader(pred_dataset)
for xi, yi in pred_loader:
    xi = xi.to(device=device, dtype = torch.float)
    yi = yi.to(device=device, dtype = torch.long)
    scores = model1(xi) 
    _, pred = scores.max(dim=1) 
    print("Paciente " + ("CON" if pred else "SIN") + " TDAH")
    if pred == yi:
      print("PREDICTED CORRECTLY!")
    else:
      print("Incorrect prediction :(")

"""## Plot randomly selected EEG"""
